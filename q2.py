...
def horizontallyShow(self, inWord):
    criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
    calibaration2(self)

    word = []
    text = inWord.upper()
    lib = os.path.join(self.loadAllFile, "Q2_Image/Q2_db/alphabet_db_onboard.txt")
    ...
    for i in range(len(self.files)):
        ...
        for j in range(len(word)):
            ...
            img = draw(img, img_points, len(word[j]))
...
def verticallyShow(self, inWord):
    criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
    calibaration2(self)

    word = []
    text = inWord.upper()
    lib = os.path.join(self.loadAllFile, "Q2_Image/Q2_db/alphabet_db_vertical.txt")
    fs = cv2.FileStorage(lib, cv2.FILE_STORAGE_READ)
